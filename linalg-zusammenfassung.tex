...
\subsubsection{Mulitilieare Algebra}
\begin{theorem}
Zu $K$-Verktorräumen $(V_1,...,V_k)$ gibt es genau einen $K$-Vektorraum $(V_1  \otimes ...  \otimes V_k)$ zusammen mit einer universellen mulitliearen Abbildung 
$$
\eta : V_1 \times ... \times V_k  \rightarrow V_1\otimes ... \otimes V_k ,   (v_1,...,v_k) \mapsto v_1\otimes...\otimes v_k
$$
d.h. zu jeder multiliearen Abblidung
$$
\xi: V_1 \times ... \times V_k \rightarrow W
$$
gibts es geanu eine linearen Abblidung $\xi_\otimes$ derart, daß das Diagramm 
\begin{gather*}
...
(v_1^{(1)},...,v_{rj}^{(k)}), j = 1,...,k,
$$
so ist eine Basis von $(V_1  \otimes ...  \otimes V_k)$ gegeben durch die Produkte
$$
(v_{i1}^{(1)} \otimes...\otimes v_{ik}^{(k)}) \textit{ mit }  1 \leq (i_j) \leq (r_j) 
$$ 
Insbesondere ist dim$(V_1\otimes...\otimes V_k) =$ dim$(V_1\cdot ... \cdot V_k)$
\end{theorem}
...
\begin{remark}
Ist $\xi$ alternieren und $ \sigma  \in S_k$, so ist
$$ 
...
\begin{theorem}
Zu $K$-Verktorräumen und einer natürlichen Zahl $k \leq 1$ gibt es einen $K$-Verktorraum
$\wedge^k$  zusammen mit einer universellen alternierenden Abbildung 
$$
\wedge: V^k \rightarrow \wedge ^k W
$$
d.h. zu jeder alternierenden Abblidung 
$$
\xi: V^k \rightarrow W
$$
gibt es genau eine linerae Abblidung $\xi_{\wedge}$ derart, daß das Diagramm 


\begin{gather*}
  \xymatrix{ (V ^ {k}) \ar[d]|{\wedge} \ar[r]|{\xi} & W\\
    (\wegde^{k} V )\ar[ur]|{\xi_\wedge} & }
\end{gather*}

kommutiert. Ist $(v_1,...,v_n)$ eine Basis von $V$, so ist eine Basis $\wegde ^k V$ gegeben durch die Produkte 
$$
v_{i1} \wegde ... \wedge v_{ik} \textit{mit} 1 \leq i1 < ... <  ik \leq n
$$
Insbesodere ist dim$\wedge^k = \binom{n}{k}$ für $ 1 \leq k \leg n = \mathrm{dim}V$. Für $k>n$ setzt man 
$$
\wedge^k = n
$$
